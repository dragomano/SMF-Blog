...
В Alpha 6 правки для новой темы стали накатывать волнами, сгенерированными с помощью Claude Code — возможно, это ускорит процесс.
...
```xml
<install for="3.0 Alpha 6 - 3.0.99">
...
Наконец-то появились тесты для исходного кода. Это снижает риск того, что каждый новый фикс что-то незаметно сломает. Также добавили конфиг для Docker.
